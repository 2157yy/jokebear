...
class EntityExtractor:
    ...
    def __init__(self, llm_client=None, model=None, tokenizer=None, api_key=None):
        """
        初始化实体抽取器
        ...
        Args:
            ...
            api_key: Qwen API Key（可选，优先使用环境变量 DASHSCOPE_API_KEY）
        """
    ...
    def extract(self, conversation: str, use_local_model: bool = False) -> List[Dict[str, Any]]:
        ...
        if self._qwen_client is not None:
            response = self._generate_qwen(prompt)
        elif use_local_model and self.model is not None and self.tokenizer is not None:
    ...
    def _generate_qwen(self, prompt: str) -> str:
        """调用 Qwen API 生成实体抽取结果"""
        response = self._qwen_client.chat.completions.create(
            model="qwen-plus",
            messages=[
                {"role": "system", "content": "你是一个实体抽取助手，只返回 JSON 格式的结果，不要输出任何其他内容。"},
